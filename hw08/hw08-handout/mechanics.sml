...
struct
  ...
  fun accOnPoint (p1 : Plane.point,
      ...
      case Plane.pointEqual(p1 , p2) of
        ...
        | false =>
              let
                  val disp12 = Plane.vecFromPoints(p1, p2)
                  val r = Plane.mag disp12
                  val aMag = (G * m2) / (r * r + softening * softening)
                  val aDir = Plane.unitVec disp12
              in
                  Plane.scale(aDir, aMag)
              end
  ...
  fun stepBody ((m, p, v) : body, a : Plane.vec, t : Scalar.scalar) : body =
      let
        val v_new = Plane.add(v, Plane.scale(a, t))
      in
        (m,
         Plane.displace (p, Plane.scale(v_new, t)), (* leapfrog *)
         v_new)
      end
