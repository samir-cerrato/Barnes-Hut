...
sig
    ...
    (* run_file filename num_iters timestep 
       ...
       For example:
          run_file( "data/galaxy2.txt", 2000, (1.0/10.0)); 
       ...
       *)
    val run_file : string * int * real -> unit
...
struct
    ...
    struct
        ...
        val timestep_1day = (86400.0) 
        val timestep_10days = (864000.0) 

        (* assumes length of bod <= 9 *)
        fun run_test (bod : Mechanics.body Seq.seq, num_iters : int, timestep : Scalar.scalar, view_radius : string, prefix : string, outfilename : string) : unit =
            let
                val header = planet_size
                           ^ view_radius
                           ^ String.concatWith "," (List.take (solar_colors, Seq.length bod)) ^ "\n"

                val () = print ("made " ^ (prefix ^ "/" ^ outfilename) ^ " ...")
                val t1 = Time.now ();
                val () = Simulation.run BarnesHut.accelerations header bod timestep num_iters (prefix ^ "/" ^ outfilename)
                val t2 = Time.now ();
        ...
        fun run_tests () =
            let
                ...
                fun prep solars bef (x,y) =
                    (x,
                     solars,
                     String.concatWith "." ["real", bef, y, "auto", "txt"])
            ...
            in
                ...
                List.app (fn (numdays, bods, outfilename) => run_test (bods, numdays, timestep_10days, solar_radius, "tests", outfilename)) all
            end
    end
    ...
    struct
                        ...
                        [pxs , pys, vxs, vys, ms, rs, gs, bs] => 
                            (case (Real.fromString pxs, Real.fromString pys,
                                 ...
                                 (SOME px, SOME py, SOME vx, SOME vy, SOME m, SOME r, SOME g, SOME b) =>
                                     ((m,
                                       Plane.fromcoord(px, py),
                                       Plane.vecFromPoints(Plane.origin, Plane.fromcoord(vx, vy))),
                                      (r,g,b))
        ...
        fun run_file (input : string, num_iters : int, timestep : Scalar.scalar): unit = 
            let 
        ...
        (* run all the data files with some suggested parameters.
           ...
           *)  
        fun run_files () : unit =
            (run_file ("data/test3.txt", 1000, (1.0/10.0));
             run_file ("data/test1.txt", 1000, (1.0/10.0));
             run_file ("data/test5.txt", 1000, (1.0/10.0));
             run_file ("data/test4.txt", 1000, (1.0/10.0));
             run_file ("data/test2.txt", 1000, (1.0/10.0));
             run_file ("data/planets.txt", 1000, (1.0/10.0));
             run_file ("data/asteroids1000.txt", 1000, (1.0/10.0)); (* 62 seconds*)
             run_file ("data/cluster2582.txt", 2000, (1.0/10.0)); (* 555 seconds *)
             run_file ("data/galaxy1.txt", 2000, (1.0/10.0)); (* 130 seconds *)
             run_file ("data/galaxy2.txt", 2000, (1.0/10.0)); (* 83 seconds *)
             run_file ("data/galaxy3.txt", 1500, (1.0/10.0)); (* 304 seconds *)
             run_file ("data/galaxy4.txt", 2000, (1.0/10.0)); (* 41 seconds *)
             run_file ("data/spiralgalaxy.txt", 2000, (1.0/10.0)); (* 94 seconds *)
             run_file ("data/galaxymerge1.txt", 5000, (1.0/5.0)); (* 820 seconds *)
             run_file ("data/galaxymerge2.txt", 2500, (1.0/10.0)); (* 626 seconds *)
             run_file ("data/galaxymerge3.txt", 2500, (1.0/10.0)); (* 655 seconds *)
             run_file ("data/galaxyform2500.txt", 2000, (1.0/10.0)); (* 294 seconds *)
             run_file ("data/collision2.txt", 2500, (1.0/10.0)); (* 330 seconds *)
             run_file ("data/collision1.txt", 1500, (1.0/10.0)); (* 299 seconds *)
             run_file ("data/saturnrings.txt", 100, (1.0/100.0)); (*  112 seconds *)
             run_file ("data/galaxy10k.txt", 100, (1.0/10.0)); (* 93 seconds *)
             run_file ("data/galaxy20k.txt", 50, (1.0/10.0)); (* 188 seconds *)
             run_file ("data/galaxy30k.txt", 800, (1.0/10.0)) (* 1736 seconds *)
             )
      end

    fun run_solar (days : int, outfilename : string) : unit =
        Test.run_test (TestData.solar_system, days, Test.timestep_10days, Test.solar_radius, "data", outfilename);

    fun run_solar_inner (days : int, outfilename : string) : unit =
        Test.run_test (TestData.solar_system, days, Test.timestep_1day, Test.small_radius,  "data", outfilename);
